...
from .utils import sample_uniform_in_simjeb_bounds, sample_uniform_points_in_sphere
...
class SurfacePointCloud:
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, ginn_bsize=1000000, return_gradients=False):
        if query_points.shape[0] <= ginn_bsize:
        ...
        n_batches = int(math.ceil(query_points.shape[0] / ginn_bsize))
        batches = [
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False,
                                radius=1, surface_pts_ratio=47/50):
        query_points = []
        surface_sample_count = int(number_of_points * surface_pts_ratio) // 2
        ...
        # NOTE: we sample directly within the bounds instead of the unit sphere
        # unit_sphere_points = sample_uniform_points_in_sphere(unit_sphere_sample_count, radius=radius)
        unit_sphere_points = sample_uniform_in_simjeb_bounds(unit_sphere_sample_count)
