...
def sample_uniform_points_in_sphere(amount, radius=1):
    unit_sphere_points = np.random.uniform(-radius, radius, size=(amount * 2 + 20, 3))
    unit_sphere_points = unit_sphere_points[np.linalg.norm(unit_sphere_points, axis=1) < radius]
...
def sample_uniform_in_simjeb_bounds(amount):
    bounds = np.load('GINN/simJEB/data/bounds.npy')
    return np.random.uniform(bounds[:,0], bounds[:,1], size=(amount, 3))